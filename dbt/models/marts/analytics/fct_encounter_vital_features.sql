    ...
    select
        ...
        max(
            case
                ...
                when loinc_code = '{{ var("vital_sign_loinc_codes")["heart_rate"] }}' and (value <= 40 or value >= 131) then 1
                when loinc_code = '{{ var("vital_sign_loinc_codes")["respiratory_rate"] }}' and (value <= 8 or value >= 25) then 1
                when loinc_code = '{{ var("vital_sign_loinc_codes")["spo2"] }}' and value <= 91 then 1
                when loinc_code = '{{ var("vital_sign_loinc_codes")["systolic_bp"] }}' and value <= 90 then 1
                else 0
            end
