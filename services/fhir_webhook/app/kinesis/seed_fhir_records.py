...
def build_quantity_observation(
    observation_id: str, loinc_code: str, display: str, value: float, unit: str, ucum_code: str, effective_datetime: datetime
) -> dict:
    return {
...
def build_blood_pressure_observation(observation_id: str, systolic: float, diastolic: float, effective_datetime: datetime) -> dict:
    return {
        ...
        "status": "final",
        "code": {
            "coding": [{"system": "http://loinc.org", "code": "85354-9", "display": "Blood pressure systolic and diastolic"}],
...
def build_seed_observations() -> list[dict]:
    ...
    return [
        ...
        build_quantity_observation(
            "step_11_spo2_001", "2708-6", "Oxygen saturation in Arterial blood", 97.0, "%", "%", simulation_start + timedelta(seconds=5)
        ),
        build_quantity_observation(
            "step_11_spo2_002", "2708-6", "Oxygen saturation in Arterial blood", 98.0, "%", "%", simulation_start + timedelta(seconds=6)
        ),
        build_quantity_observation(
            "step_11_spo2_003", "2708-6", "Oxygen saturation in Arterial blood", 96.0, "%", "%", simulation_start + timedelta(seconds=7)
        ),
