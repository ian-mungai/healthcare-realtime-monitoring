    ...
    if record_number not in _record_cache:
        try:
            ...
            raise HTTPException(
                status_code=502,
                detail=(
                    "Unable to retrieve BIDMC "
                    f"record {record_number} "
                    f"from PhysioNet: {exc}"
                ),
            ) from exc
...
def get_next_reading(
    ...
    if not readings:
        raise HTTPException(
            status_code=404,
            detail="No BIDMC readings available",
        )

    position = _record_position.get(
        record_number,
        0,
    )
