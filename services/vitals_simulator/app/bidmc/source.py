...
def normalize_record_number(
    ...
    """
    if not (
        SUPPORTED_RECORD_MIN
        <= record_number
        <= SUPPORTED_RECORD_MAX
    ):
        raise ValueError(
            "BIDMC record number must be between "
            f"{SUPPORTED_RECORD_MIN} and "
            f"{SUPPORTED_RECORD_MAX}"
        )
...
def build_record_name(
    ...
    into:
        ...
        bidmc01n
    """
    record_number = normalize_record_number(
        record_number
    )
...
def find_channel_index(
    ...
    normalized_candidates = {
        normalize_channel_name(candidate)
    ...
    raise ValueError(
        "Could not find any of these channels: "
        f"{sorted(candidates)}. "
        f"Available channels: {signal_names}"
    )


def fetch_remote_bidmc_record(
    record_number: int,
) -> list[VitalReading]:
    """
    ...
    print(
        f"Fetching remote PhysioNet record: "
        f"{record_name}"
    )

    signals, fields = wfdb.rdsamp(
        record_name,
        pn_dir=PHYSIONET_DIRECTORY,
    )
    ...
    if sampling_frequency <= 0:
        raise ValueError(
            "Invalid sampling frequency returned "
            "by PhysioNet"
        )

    hr_index = find_channel_index(
        signal_names,
        {
            "HR",
            "HEART RATE",
