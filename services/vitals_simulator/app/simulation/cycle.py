...
def build_simulator_event(reading: VitalReading, patient_id: str, encounter_id: str, simulation_start: datetime, bp_cadence: BloodPressureCadence) -> SimulatorEvent:
    ...
    if bp_reading is not None:
        ...
        observations.append(build_blood_pressure_observation(patient_id=patient_id, encounter_id=encounter_id, effective_datetime=effective_datetime, reading=bp_reading, source_offset_seconds=reading.offset_seconds))
    ...
    return SimulatorEvent(source_record_id=reading.source_record_id, offset_seconds=reading.offset_seconds, patient_id=patient_id, encounter_id=encounter_id, observation_count=len(observations), observations=observations)
