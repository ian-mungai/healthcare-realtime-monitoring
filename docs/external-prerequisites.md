...
| GitHub repository environment | Repository approvals and encrypted settings belong to GitHub | Configure `AWS_REGION` as non-sensitive configuration and store `AWS_DEPLOY_ROLE_ARN`, `TF_STATE_BUCKET`, `TF_STATE_PREFIX` and `TF_STATE_REGION` as protected secrets | Remove separately if the repository is retired |
