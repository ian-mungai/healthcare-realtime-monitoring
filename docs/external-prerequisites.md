...
| Private deployment configuration | Account-specific Terraform inputs must remain outside public GitHub configuration | Synchronize the ignored Terraform inputs to the encrypted, versioned `<project-name>/terraform/config/` state-bucket prefix | Remove with the persistent state bucket only after all environments are retired |
