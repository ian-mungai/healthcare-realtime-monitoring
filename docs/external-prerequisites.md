...
| Persistent Terraform state bucket | The main stack cannot safely manage the bucket containing its own state | Create once with `infra/bootstrap`; retain its local bootstrap state securely | Survives application teardown by design |
...
| Local build toolchain | Containers, Synthea, packages, and Terraform run outside AWS | Install Python 3.12, Terraform 1.11+, AWS CLI, Docker, Java 17, and Gradle | No AWS teardown action |
