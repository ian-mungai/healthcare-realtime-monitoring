...
def sample_patient():
    return {
        ...
        "identifier": [
            {
                "system": SYNTHEA_IDENTIFIER_SYSTEM,
                "value": "synthea-patient-1",
            }
        ],
        "managingOrganization": {
            "reference": "Organization?identifier=test"
        },
        "generalPractitioner": [
            {
                "reference": (
...
def older_encounter():
    return {
        ...
        "identifier": [
            {
                "system": SYNTHEA_IDENTIFIER_SYSTEM,
                "value": "synthea-encounter-old",
            }
        ],
        "subject": {
            "reference": "urn:uuid:patient-full-url"
        },
        "period": {
...
def sample_encounter():
    return {
        ...
        "location": [
            {
                "location": {
                    "reference": "Location?identifier=test"
                }
            }
        ],
    }
...
def sample_bundle(
    ...
    return {
        ...
        "entry": [
            {
                "fullUrl": "urn:uuid:patient-full-url",
            ...
            {
                "fullUrl": "urn:uuid:old-encounter-full-url",
                "resource": older_encounter,
                "request": {
                    "method": "POST",
                    "url": "Encounter",
                },
            ...
            {
                "fullUrl": "urn:uuid:encounter-full-url",
...
def test_sanitize_patient_does_not_modify_original(
    sample_patient,
):
...
def test_sanitize_encounter(sample_encounter):
    ...
    assert encounter["subject"]["reference"] == (
        "urn:uuid:patient-full-url"
    )


def test_sanitize_encounter_does_not_modify_original(
    sample_encounter,
):
...
def test_select_seed_resources_chooses_patient_and_latest_encounter(
    sample_bundle,
):
    patient, encounter = select_seed_resources(
        sample_bundle
    )
...
def test_select_seed_resources_sanitizes_patient(
    sample_bundle,
):
    patient, _ = select_seed_resources(
        sample_bundle
    )
...
def test_select_seed_resources_sanitizes_encounter(
    sample_bundle,
):
    _, encounter = select_seed_resources(
        sample_bundle
    )
...
def test_select_seed_resources_requires_patient():
    ...
    with pytest.raises(
        RuntimeError,
        match="No Patient resource found",
    ):
...
def test_select_seed_resources_requires_encounter():
    ...
    with pytest.raises(
        RuntimeError,
        match="No Encounter resources found",
    ):
        select_seed_resources(bundle)


def test_find_conditional_references(
    sample_encounter,
):
    references = find_conditional_references(
        sample_encounter
    )
    ...
    assert (
        "Location?identifier=test"
        in references
    )


def test_sanitized_encounter_has_no_conditional_references(
    sample_encounter,
):
    encounter = sanitize_encounter(
        sample_encounter
...
def test_get_synthea_identifier(
    sample_patient,
):
    system, value = get_synthea_identifier(
        sample_patient
    )
...
def test_get_synthea_identifier_missing():
    ...
    with pytest.raises(
        RuntimeError,
        match="does not contain a usable identifier",
    ):
        get_synthea_identifier(resource)


def test_extract_id_from_relative_location():
    resource_id = extract_id_from_location(
        "Patient/137506799/_history/1"
    )
...
def test_search_resource_by_identifier_found():
    ...
    resource = search_resource_by_identifier(
        "Patient",
        SYNTHEA_IDENTIFIER_SYSTEM,
        "synthea-patient-1",
    )
...
def test_get_resource():
    route = respx.get(
        f"{FHIR_BASE_URL}/Patient/137506799"
    ).mock(
        return_value=httpx.Response(
            200,
            json={
...
@respx.mock
def test_ensure_patient_exists_reuses_existing(
    sample_patient,
):
...
@respx.mock
def test_ensure_patient_exists_creates_when_missing(
    sample_patient,
):
    search_route = respx.get(
        f"{FHIR_BASE_URL}/Patient"
...
@respx.mock
def test_ensure_encounter_exists_reuses_existing(
    sample_encounter,
):
    search_route = respx.get(
    ...
    result = ensure_encounter_exists(
        encounter,
        "137506799",
    )
...
@respx.mock
def test_ensure_encounter_exists_creates_when_missing(
    sample_encounter,
):
    search_route = respx.get(
        f"{FHIR_BASE_URL}/Encounter"
    ...
    assert submitted["subject"]["reference"] == (
        "Patient/137506799"
    )