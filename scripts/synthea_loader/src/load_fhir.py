...
def find_patient_bundles() -> list[Path]:
    ...
    for file_path in FHIR_OUTPUT_DIR.glob("*.json"):
        ...
        if (
            ...
            and contains_resource(bundle, "Patient")
        ):
...
def select_seed_resources(
    ...
    if patient is None:
        raise RuntimeError(
            "No Patient resource found in "
            "Synthea bundle"
        )

    if not encounters:
        raise RuntimeError(
            "No Encounter resources found in "
            "Synthea bundle"
        )

    most_recent_encounter = max(
        encounters,
...
def find_conditional_references(
    value,
) -> set[str]:
    """
...
def get_synthea_identifier(
    ...
    raise RuntimeError(
        ...
        "does not contain a usable identifier"
    )


def search_resource_by_identifier(
    resource_type: str,
    system: str,
    value: str,
) -> dict | None:
    ...
    if len(entries) > 1:
        print(
            f"WARNING: found {len(entries)} "
            f"{resource_type} resources for "
            f"identifier {system}|{value}."
        )

        print(
            "Using the first matching resource."
        )
...
def create_resource(
    ...
    if not location:
        raise RuntimeError(
            f"HAPI created {resource_type} "
            "but returned neither a resource "
            "body nor a Location header"
        )
...
def ensure_encounter_exists(
    ...
    if conditional_references:
        raise RuntimeError(
            "Encounter still contains "
            "unsupported conditional references: "
            f"{sorted(conditional_references)}"
        )
...
def main():
    ...
    if not bundles:
        raise RuntimeError(
            "No patient FHIR Bundles "
            f"found in {FHIR_OUTPUT_DIR}"
        )

    print(
        f"FHIR server: {FHIR_BASE_URL}"
    )

    print(
        f"FHIR output directory: "
        f"{FHIR_OUTPUT_DIR}"
    )
    ...
    for bundle_path in bundles:
        ...
        if not synthea_patient_id:
            raise RuntimeError(
                "Synthea Patient has no id"
            )

        if not synthea_encounter_id:
            raise RuntimeError(
                "Synthea Encounter has no id"
            )
        ...
        print(
            "  Patient:"
            f" {synthea_patient_id}"
        )

        print(
            "  Encounter:"
            f" {synthea_encounter_id}"
        )

        patient_conditional_refs = (
            find_conditional_references(
                patient
            )
        )

        encounter_conditional_refs = (
            find_conditional_references(
                encounter
            )
        )
        ...
        if patient_conditional_refs:
            raise RuntimeError(
                "Sanitized Patient still "
                "contains conditional references: "
                f"{sorted(patient_conditional_refs)}"
            )

        if encounter_conditional_refs:
            raise RuntimeError(
                "Sanitized Encounter still "
                "contains conditional references: "
                f"{sorted(encounter_conditional_refs)}"
            )
        ...
        hapi_encounter_id = (
            hapi_encounter.get("id")
        )

        if not hapi_encounter_id:
            raise RuntimeError(
                "HAPI Encounter has no id"
            )
        ...
        print(
            "  Patients:"
            f" {len(resource_map['patients'])}"
        )

        print(
            "  Encounters:"
            f" {len(resource_map['encounters'])}"
        )
